...
(function($) {
  $.fn.joyride = function(options) {
     ...
     */
    getParentElement = function($tip) {
      
      if ($tip.data('anchor-text') && $tip.data('anchor-text').length > 0) {
        
        var anchorhtml = $tip.data('anchor-text');
        $.each($('a'), function(i) {
    ...
    showNextTip = function() {
      //alert('show next tip');
      var $tip = $(tipContent[count]);
      
        var parentElement = getParentElement($tip),
            opt = {};
        
        if (!parentElement) return;
        
        // Parse the options string
        $.each(($tip.data('options') || ':').split(';'),
          function (i, s) {
            var p = s.split(':');
            if (p.length == 2) {
